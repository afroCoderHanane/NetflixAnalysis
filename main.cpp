...
//CONSIDER ONLY MyMovieStat.cpp and MyMovieStat.h
...
//	cout << "Choose the size of the array to get the mode. \t ";
  ...
  ofstream output;
	output.open("output.dat", ios::out);
  ...
  if (size<=1)throw string("please enter a valid positive integer superior to 0 & 1");
   ...
   exit(0);
	...
	// 	// 	 << endl << endl;
	// 	 output<< endl << "At least input a size(integer) that's 2 or more.  "
	// 		 << endl << endl;
		
	// 	cout << "Choose the size of the array to get the mode. \t ";
	//   output << "Choose the size of the array to get the mode. \t ";
	// 	//cin >> size;
	// }
 // while(inputdata>>)
  ...
  //uncomment this section to use the functions
	...
	output << endl << "The array is:" << endl << endl;
	...
	if(mode_array == -1)
	{
		output << "There is no mode from this array." << endl;

		return 0;
	}

	output << "The mode from this array is: " << mode_array << endl;
...
// 	//cout << "How many student did you want to test \t ";
	
// 	cin >> size;
//   if (size<=1)
//   { cout<<"please enter a valid positive integer superior to 0 & 1"<<endl;
//     cin>> size;
//   }
...
// 	//as the name suggests, it shows the array to the user
// 	showArray(modeA, size);
//   //int n = sizeof(modeA)/sizeof(modeA[0]);
//   findMedian(modeA,size);
//   average(modeA, size);
// 	//declares and determines the mode of the array
//     int mode_array = mode(modeA, size);
